...
2. Make sure PHP has write access to the directory `/secure` - This is where the password hash will be stored (see below). Write access can be achieved by either changing the folder's chmod to 777 (less secure) or making sure that the owner of the file is the same as the one the server is using.
3. Create a configuration file (rename or copy the `config_example.json`).
4. Open Monolog Viewer in the browser
5. Enter a password of your choice twice and click "Create login".
6. Done. You can now log in and use your installation of Monolog Viewer
